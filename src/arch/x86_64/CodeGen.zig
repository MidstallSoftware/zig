    ...
    fn getSymbolIndex(owner: Owner, ctx: *Self) !u32 {
        switch (owner) {
            .func_index => |func_index| {
                const zcu = ctx.bin_file.comp.module.?;
...
pub const MCValue = union(enum) {
    ...
    pub fn format(
        ...
        switch (mcv) {
            ...
            .load_symbol => |pl| try writer.print("[sym:{} + 0x{x}]", .{ pl.sym, pl.off }),
            .lea_symbol => |pl| try writer.print("sym:{} + 0x{x}", .{ pl.sym, pl.off }),
...
fn airTrunc(self: *Self, inst: Air.Inst.Index) !void {
    ...
    const result = result: {
        ...
        if (dst_ty.zigTypeTag(zcu) == .Vector) {
            ...
            const mask_val = try zcu.intValue(src_elem_ty, @as(u64, math.maxInt(u64)) >> @intCast(64 - dst_info.bits));
            ...
            .Vector => switch (lhs_ty.childType(zcu).zigTypeTag(zcu)) {
                .Int => if (@as(?Mir.Inst.FixedTag, switch (lhs_ty.childType(zcu).intInfo(zcu).bits) {
                    ...
                    .splat => {
                        ...
                        const mask_mcv = try self.genTypedValue(Value.fromInterned(try zcu.intern(.{ .aggregate = .{
                            .ty = mask_ty.toIntern(),
...
fn airStructFieldVal(self: *Self, inst: Air.Inst.Index) !void {
    ...
    const result: MCValue = result: {
        ...
        const field_off: u32 = switch (container_ty.containerLayout(zcu)) {
            ...
            .@"packed" => if (zcu.typeToStruct(container_ty)) |struct_obj| zcu.structPackedFieldBitOffset(struct_obj, extra.field_index) else 0,
        };
...
fn airFieldParentPtr(self: *Self, inst: Air.Inst.Index) !void {
    ...
    const field_off: i32 = switch (parent_ty.containerLayout(zcu)) {
        ...
        .@"packed" => @divExact(@as(i32, inst_ty.ptrInfo(zcu).packed_offset.bit_offset) +
            (if (zcu.typeToStruct(parent_ty)) |struct_obj| zcu.structPackedFieldBitOffset(struct_obj, extra.field_index) else 0) -
    ...
    if (switch (tag) {
        ...
        switch (tag) {
            ...
            .div_trunc, .div_floor, .div_exact, .rem, .mod => switch (src_info.signedness) {
                ...
                .unsigned => {
                    const dst_mcv = try self.allocRegOrMemAdvanced(dst_ty, maybe_inst, false);
                    const manyptr_u32_ty = try zcu.ptrType(.{
                        .child = .u32_type,
                        .flags = .{
                            .size = .Many,
                        },
                    });
                    const manyptr_const_u32_ty = try zcu.ptrType(.{
                        .child = .u32_type,
                        .flags = .{
                            .size = .Many,
                            .is_const = true,
            ...
            switch (lhs_ty.childType(zcu).zigTypeTag(zcu)) {
                .Int => switch (air_tag) {
                    ...
                    => {
                        ...
                        const not_mcv = try self.genTypedValue(try unsigned_ty.maxInt(zcu, unsigned_ty));
        ...
        .air => |callee| if (try self.air.value(callee, zcu)) |func_value| {
            ...
            switch (switch (func_key) {
                ...
                .ptr => |ptr| if (ptr.byte_offset == 0) switch (ptr.base_addr) {
                    .decl => |decl| ip.indexToKey(zcu.declPtr(decl).val.toIntern()),
                    else => func_key,
                } else func_key,
            }) {
                .func => |func| {
                ...
                .extern_func => |extern_func| {
                    ...
                    const lib_name = extern_func.lib_name.toSlice(ip);
                    const decl_name = owner_decl.name.toSlice(ip);
...
fn genSetReg(
    ...
    switch (src_mcv) {
        ...
        .undef => if (opts.safety) switch (dst_reg.class()) {
            ...
            .segment, .x87, .mmx, .sse => try self.genSetReg(dst_reg, ty, try self.genTypedValue(try zcu.undefValue(ty)), opts),
        },
...
fn airSelect(self: *Self, inst: Air.Inst.Index) !void {
    ...
    const result = result: {
        ...
        const mask_reg = mask: {
            ...
            } else return self.fail("TODO implement airSelect for {}", .{ty.fmt(zcu)});
            const elem_bits: u16 = @intCast(elem_abi_size * 8);
            const mask_elem_ty = try zcu.intType(.unsigned, elem_bits);
            const mask_ty = try zcu.vectorType(.{ .len = vec_len, .child = mask_elem_ty.toIntern() });
            if (!pred_fits_in_elem) if (self.hasFeature(.ssse3)) {
                var mask_elems: [32]InternPool.Index = undefined;
                for (mask_elems[0..vec_len], 0..) |*elem, bit| elem.* = try zcu.intern(.{ .int = .{
        ...
        }) orelse return self.fail("TODO implement airSelect for {}", .{ty.fmt(zcu)});
        if (has_avx) {
...
fn airShuffle(self: *Self, inst: Air.Inst.Index) !void {
    ...
    for (mask_elems, 0..) |*mask_elem, elem_index| {
        const mask_elem_val =
            Value.fromInterned(extra.mask).elemValue(zcu, elem_index) catch unreachable;
    ...
    const result = @as(?MCValue, result: {
        ...
        blendv: {
            ...
            const select_mask_elem_ty = try zcu.intType(.unsigned, elem_abi_size * 8);
            const select_mask_ty = try zcu.vectorType(.{
            ...
            for (
                ...
                select_mask_elem.* = (if (mask_elem < 0)
                    try select_mask_elem_ty.maxIntScalar(zcu, select_mask_elem_ty)
                else
                    try select_mask_elem_ty.minIntScalar(zcu, select_mask_elem_ty)).toIntern();
            }
            const select_mask_mcv = try self.genTypedValue(Value.fromInterned(try zcu.intern(.{ .aggregate = .{
        ...
        pshufb: {
            ...
            for (lhs_mask_elems[0..max_abi_size], 0..) |*lhs_mask_elem, byte_index| {
                ...
                lhs_mask_elem.* = try zcu.intern(.{ .int = .{
                    .ty = .u8_type,
            ...
            const lhs_mask_ty = try zcu.vectorType(.{ .len = max_abi_size, .child = .u8_type });
            const lhs_mask_mcv = try self.genTypedValue(Value.fromInterned(try zcu.intern(.{ .aggregate = .{
            ...
            for (rhs_mask_elems[0..max_abi_size], 0..) |*rhs_mask_elem, byte_index| {
                const elem_index = byte_index / elem_abi_size;
                rhs_mask_elem.* = try zcu.intern(.{ .int = .{
                    .ty = .u8_type,
                    .storage = .{ .u64 = if (elem_index >= mask_elems.len) 0b1_00_00000 else elem: {
                        const mask_elem = mask_elems[elem_index] orelse break :elem 0b1_00_00000;
                        if (mask_elem >= 0) break :elem 0b1_00_00000;
            ...
            const rhs_mask_ty = try zcu.vectorType(.{ .len = max_abi_size, .child = .u8_type });
            const rhs_mask_mcv = try self.genTypedValue(Value.fromInterned(try zcu.intern(.{ .aggregate = .{
        ...
        rhs_ty.fmt(zcu),
        dst_ty.fmt(zcu),
        Value.fromInterned(extra.mask).fmtValue(zcu, null),
    });
...
fn genTypedValue(self: *Self, val: Value) InnerError!MCValue {
    const zcu = self.bin_file.comp.module.?;
    return switch (try codegen.genTypedValue(self.bin_file, self.src_loc, val, self.owner.getDecl(zcu))) {
        .mcv => |mcv| switch (mcv) {
            .none => .none,
            .undef => .undef,
            .immediate => |imm| .{ .immediate = imm },
...
fn splitType(self: *Self, ty: Type) ![2]Type {
    ...
    if (classes.len == 2) for (&parts, classes, 0..) |*part, class, part_i| {
        part.* = switch (class) {
            ...
            .float => Type.f32,
            .float_combine => try zcu.arrayType(.{ .len = 2, .child = .f32_type }),
            .sse => Type.f64,
            else => break,
        };
...
const Temp = struct {
    ...
    fn getOffset(temp: Temp, offset: i32, self: *Self) !Temp {
        ...
        switch (temp.tracking(self).short) {
            ...
            .lea_frame => |frame_addr| new_temp_index.tracking(self).* =
                InstTracking.init(.{ .lea_frame = frame_addr }),
        }
        return .{ .index = new_temp_index.toIndex() };
    }

    fn toOffset(temp: *Temp, offset: i32, self: *Self) !void {
        if (offset == 0) return;
        ...
        const new_temp = try temp.getOffset(offset, self);
        try temp.die(self);
        temp.* = new_temp;
    }
    ...
    fn move(temp: Temp, inst: Air.Inst.Index, self: *Self) !void {
        if (self.liveness.isUnused(inst)) try temp.die(self) else switch (temp.unwrap(self)) {
            ...
            .temp => |temp_index| {
                ...
                for (temp_tracking.short.getRegs()) |reg| {
                    if (RegisterManager.indexOfRegIntoTracked(reg)) |tracked_index| {
                        self.register_manager.registers[tracked_index] = inst;
                    }
                }
                temp_tracking.short = .{ .dead = self.scope_generation };
            },
        }
    }
...
fn tempFromValue(self: *Self, value: MCValue) Temp {
...
fn tempFromOperand(
    ...
    if (!self.liveness.operandDies(inst, op_index)) {
        ...
        const ip_index = op_ref.toInterned().?;
        const gop = try self.const_tracking.getOrPut(self.gpa, ip_index);
        if (!gop.found_existing) gop.value_ptr.* = InstTracking.init(init: {
            const const_mcv = try self.genTypedValue(Value.fromInterned(ip_index));
            switch (const_mcv) {
        ...
        return self.tempFromValue(gop.value_ptr.short);
    }
    ...
    switch (tracking.short) {
        .register, .register_pair, .register_overflow => for (tracking.short.getRegs()) |reg| {
            if (RegisterManager.indexOfRegIntoTracked(reg)) |index| {
                self.register_manager.registers[index] = temp_index.toIndex();
            }
        },
        .load_frame => |frame_addr| if (frame_addr.index.isNamed()) return .{ .index = op_ref.toIndex().? },
        else => {},
    }
    switch (tracking.short) {
        .eflags, .register_overflow => self.eflags_inst = temp_index.toIndex(),
        else => {},
    }

    temp_index.tracking(self).* = tracking.*;
    tracking.reuse(self, temp_index.toIndex(), op_inst);
